...
import { Workout, TrainingSession, SetData, Exercise, ActiveSessionData } from '../types';
...
const TrainingSessionView: React.FC<TrainingSessionViewProps> = ({ 
  ...
  const [currentExIndex, setCurrentExIndex] = useState(initialData?.currentExIndex || 0);
  const [currentStepIndex, setCurrentStepIndex] = useState(initialData?.currentStepIndex || 0); 
  const [exerciseResults, setExerciseResults] = useState<Record<string, SetData[]>>(initialData?.exerciseResults || {});
  ...
  useEffect(() => {
    if (!exerciseResults[currentExercise.id]) {
      ...
      setExerciseResults(prev => ({ ...prev, [currentExercise.id]: initialSets }));
    }
  ...
  }, [exerciseResults, currentExIndex, currentStepIndex, startTime, workout.id, onUpdate]);

  const currentSetData = exerciseResults[currentExercise.id]?.[currentStepIndex];
  ...
  if (!currentSetData) return <div className="h-screen bg-black flex items-center justify-center text-[#8E8E93] font-mono">LOADING...</div>;

  const previousPerformance = lastSession?.exerciseResults[currentExercise.id]?.[currentStepIndex];
  ...
  return (
    <div className="flex flex-col h-screen bg-black text-white overflow-hidden relative">
      ...
      {showOverview && (
        <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-xl animate-in flex flex-col">
          <div className="flex justify-between items-center px-6 pt-12 pb-4 border-b border-white/10">
            <h2 className="text-xl font-black tracking-tight">Workout Overview</h2>
            <button 
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            {workout.exercises.map((ex, idx) => {
              ...
              return (
                ...
                >
                   <div>
                     <p className={`text-[10px] font-black uppercase tracking-widest ${isCurrent ? 'text-black/60' : 'text-[#8E8E93]'}`}>
                       {idx + 1}. {ex.hasWarmup ? 'Warmup + ' : ''}{ex.sets} Sets
                     </p>
                     <h3 className="font-bold text-lg">{ex.name}</h3>
                   </div>
                   <div className="flex items-center space-x-3">
                     <span className={`font-mono text-sm font-bold ${isCurrent ? 'text-black' : 'text-[#8E8E93]'}`}>
                       {completedSets}/{totalSets}
                     </span>
                     {isDone && (
                       <div className={`w-6 h-6 rounded-full flex items-center justify-center ${isCurrent ? 'bg-black text-[#FF9500]' : 'bg-[#32D74B] text-black'}`}>
      ...
      <div className="flex justify-between items-center px-6 pt-12 pb-4">
        ...
        <button onClick={onCancel} className="text-[#8E8E93] active:scale-90 transition-transform">
      ...
      <div className="flex-1 flex flex-col px-6">
        <div className="mt-8">
          <h1 className="text-4xl font-black tracking-tighter leading-none mb-2 break-words">
            {currentExercise.name}
          </h1>
          <h2 className="text-xl font-bold text-[#FF9500] italic uppercase tracking-tight">
            {currentStep.label}
          </h2>
        </div>

        {/* Full Screen Interactive Area */}
        <div className="flex-1 flex flex-col justify-center space-y-12 pb-20">
          {currentStep.type === 'warmup' ? (
            <div className="flex flex-col items-center justify-center space-y-8 animate-in">
              <div className="w-40 h-40 rounded-full border border-[#FF9500]/30 flex items-center justify-center bg-[#FF9500]/5">
                <i className="fa-solid fa-fire-flame-curved text-6xl text-[#FF9500]"></i>
              </div>
              <div className="text-center">
                <p className="text-[#8E8E93] font-medium max-w-[200px]">Perform 10-15 reps with very light weight.</p>
              </div>
            </div>
          ) : (
            <div className="space-y-10 animate-in">
              ...
              {previousPerformance && (
                <div className="flex justify-center">
                  <div className="inline-flex items-center space-x-3 bg-[#1C1C1E] px-4 py-2 rounded-full border border-white/5">
                    <span className="text-[10px] font-black uppercase text-[#8E8E93] tracking-wider">Goal</span>
                    <span className="text-sm font-mono font-bold text-[#FF9500]">{previousPerformance.weight}kg</span>
                    <span className="text-white/10">•</span>
                    <span className="text-sm font-mono font-bold text-[#FF9500]">{previousPerformance.reps} reps</span>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 gap-12">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                     <button 
                      onClick={() => updateCurrentSet({ weight: Math.max(0, currentSetData.weight - 2.5) })}
                      className="w-16 h-16 rounded-3xl bg-[#1C1C1E] flex items-center justify-center text-xl active:bg-[#FF9500] active:text-black transition-all"
                    >
                      <i className="fa-solid fa-minus"></i>
                    </button>
                    <div className="text-center flex-1">
                      <p className="text-[10px] font-black uppercase text-[#8E8E93] tracking-[0.3em] mb-2">Weight</p>
                      <div className="flex items-baseline justify-center">
                        <input 
                          type="number" 
                          value={currentSetData.weight}
                          onChange={(e) => updateCurrentSet({ weight: parseFloat(e.target.value) || 0 })}
                          className="w-32 bg-transparent text-6xl font-mono font-black text-center focus:outline-none"
                        />
                        <span className="text-xl font-bold text-[#8E8E93] ml-1">kg</span>
                      </div>
                    </div>
                    <button 
                      onClick={() => updateCurrentSet({ weight: currentSetData.weight + 2.5 })}
                      className="w-16 h-16 rounded-3xl bg-[#1C1C1E] flex items-center justify-center text-xl active:bg-[#FF9500] active:text-black transition-all"
                    >
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                     <button 
                      onClick={() => updateCurrentSet({ reps: Math.max(0, currentSetData.reps - 1) })}
                      className="w-16 h-16 rounded-3xl bg-[#1C1C1E] flex items-center justify-center text-xl active:bg-[#FF9500] active:text-black transition-all"
                    >
                      <i className="fa-solid fa-minus"></i>
                    </button>
                    <div className="text-center flex-1">
                      <p className="text-[10px] font-black uppercase text-[#8E8E93] tracking-[0.3em] mb-2">Reps</p>
                      <input 
                        type="number" 
                    ...
                    <button 
                      onClick={() => updateCurrentSet({ reps: currentSetData.reps + 1 })}
                      className="w-16 h-16 rounded-3xl bg-[#1C1C1E] flex items-center justify-center text-xl active:bg-[#FF9500] active:text-black transition-all"
                    >
                      <i className="fa-solid fa-plus"></i>
                    </button>
      ...
      <footer className="p-8 pb-safe space-y-6">
           ...
           {steps.map((_, i) => (
             <div 
               ...
               className={`h-1.5 flex-1 rounded-full transition-all duration-500 ${
                 i === currentStepIndex ? 'bg-white scale-y-110' : (i < currentStepIndex ? 'bg-[#FF9500]' : 'bg-[#1C1C1E]')
               }`}
             />
           ))}
        </div>
        ...
        <div className="flex space-x-4">
          ...
          >
            <span>{currentStep.type === 'warmup' ? 'Warm up Done' : 'Complete Set'}</span>
